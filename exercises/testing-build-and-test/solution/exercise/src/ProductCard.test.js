...
describe("ProductCard component", () => {
  it("displays an image", () => {
    const wrapper = shallow(
      <ProductCard item={testItem} onAddToCart={() => {}} />
    );
  ...
  it("displays a price with a currency", () => {
    ...
    expect(wrapper.find(".price").text()).toEqual(
      `$${testItem.price.toFixed(2)} ${testItem.currency}`
    );
  });
  ...
  it("displays a discounted price with a currency", () => {
    ...
    const wrapper = shallow(
      <ProductCard item={testItem} onAddToCart={() => {}} discount={discount} />
    );
    expect(wrapper.find(".price").text()).toEqual(
      `$${discountedPrice.toFixed(2)} ${testItem.currency}`
    );
  });
  ...
  it("displays all descriptions", () => {
    ...
    expect(wrapper.find(".description")).toHaveLength(
      testItem.descriptions.length
    );
  });
  ...
  it("displays a rating text", () => {
    ...
    expect(wrapper.find(".recommended").text()).toEqual(
      `Highly recommended by ${recommendationRatio}% users`
    );
  });
